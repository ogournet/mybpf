...
/*
 * linked list and hash table, from linux kernel
 */
...
#ifdef LIST_HEAD
/* Avoid conflict with <sys/queue.h> */
#undef LIST_HEAD
#endif
...
/* We do not access the list_head or hlist_head items from multiple
 * threads, and so the READ_ONCE and WRITE_ONCE functionality is not
 * needed. Likewise smp_store_release and smp_load_acquire. */
 ...
 */
static inline void __list_add(struct list_head *enew,
 ...
 */
static inline void list_add(struct list_head *enew, struct list_head *head)
{
 ...
 */
static inline void list_add_tail(struct list_head *enew, struct list_head *head)
{
...
static inline int list_is_queued(struct list_head *entry)
{
